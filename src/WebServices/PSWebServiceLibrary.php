...
namespace MpSoft\MpSizeChart\WebServices;
/**
 * @package PrestaShopWebservice
 */
...
*/
    ...
    protected function checkStatusCode($request)
    {
        switch ($request['status_code']) {
            ...
            case 500:
                ...
                break;
            default:
                throw new PrestaShopWebserviceException('This call to PrestaShop Web Services returned an unexpected HTTP status of:' . $request['status_code']);
        }
    ...
    */
    ...
    <h6 style="font-size:9pt;margin:0">'
                    . $title
                    . '</h6>
    <pre>'
    ...
    */
    protected function parseXML($response)
    {
        ...
        if ($response != '') {
            ...
            if (LIBXML_VERSION < 20900) { // Avoid load of external entities (security problem). // Required only if LIBXML_VERSION < 20900 libxml_disable_entity_loader(true); } $xml=simplexml_load_string(trim($response), 'SimpleXMLElement' , LIBXML_NOCDATA | LIBXML_NONET); if (libxml_get_errors()) { $msg=var_export(libxml_get_errors(), true); libxml_clear_errors(); throw new PrestaShopWebserviceException('HTTP XML response is not parsable: ' . $msg);
            }
    ...
    /** * Add (POST) a resource * <p>Unique parameter must take : <br><br>
    ...
    */
    public function get($options)
    {
        if (isset($options['url'])) {
            ...
            $params = ['filter', 'display', 'sort', 'limit', 'id_shop', 'id_group_shop', 'schema', 'language', 'date', 'price'];
            foreach ($params as $p) {
